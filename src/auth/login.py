...
def authenticate_user(username_input, password) -> bool:
    ...
    authenticated = False
    if username:
        log_event(f"User {username} is trying to log in", "0")
        authenticated = check_password(password, password_db)
    else:
        log_event("User failed to log in due to non-existing username", "1")
    ...
    if authenticated and role == "service_engineer":
        ...
        log_event("User logged in successfully", "0")
        return True
